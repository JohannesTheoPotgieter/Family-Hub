...
const AppInner = () => {
  ...
  if (!state.activeUserId) return <LoginScreen users={state.users} hasPin={(id) => Boolean(state.userPins[id])} isSetupComplete={(id) => state.setupCompleted[id]} onUnlock={(id, pin) => { const unlocked = verifyPin(id, pin, state.userPins[id]); if (unlocked) setState((c) => ({ ...c, activeUserId: id })); return unlocked; }} onStartSetup={(id) => setState((c) => ({ ...c, setupUserId: id }))} />;
