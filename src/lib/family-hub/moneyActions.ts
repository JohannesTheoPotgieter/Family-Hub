...
export const createEmptyBillDraft = () => ({ title: '', amount: '', dueDateIso: new Date().toISOString().slice(0, 10), category: 'Utilities', notes: '', autoCreateTransaction: true });
...
export const buildBillPayload = (draft: ReturnType<typeof createEmptyBillDraft>): Omit<Bill, 'id' | 'paid' | 'paidDateIso' | 'proofFileName' | 'linkedTransactionId'> | null => {
  ...
  return { title: draft.title.trim(), amountCents: toCents(amount), dueDateIso: draft.dueDateIso, category: draft.category, notes: draft.notes.trim() || undefined, autoCreateTransaction: draft.autoCreateTransaction };
};
...
export const getBudgetSaveMode = (budgets: Budget[], monthIsoYYYYMM: string, category: string) => findBudgetForMonthCategory({ bills: [], transactions: [], budgets, settings: { currency: 'ZAR' } }, monthIsoYYYYMM, category) ? 'update' : 'create';
