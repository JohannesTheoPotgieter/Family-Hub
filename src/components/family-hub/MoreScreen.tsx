  ...
  return (
    <section className="stack-lg">
      ...
      <article className="glass-card stack">
        ...
        <input type="password" maxLength={4} inputMode="numeric" value={newPin} onChange={(e) => setNewPin(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="New 4-digit PIN" />
        <button className="btn btn-primary" disabled={newPin.length !== 4} onClick={() => { onChangePin(activeUserId, newPin); setNewPin(''); }}>
          Change my PIN
        </button>
      </article>
      ...
      <article className="glass-card stack">
        ...
        <form className="stack" onSubmit={(e) => { e.preventDefault(); if (!placeName.trim()) return; onAddPlace(placeName.trim()); setPlaceName(''); }}>
          <input value={placeName} onChange={(e) => setPlaceName(e.target.value)} placeholder="Add first place" />
          <button className="btn btn-primary" type="submit">Save place</button>
        </form>
        {!places.length ? <div className="empty-state">No places added yet.</div> : places.map((p) => <div key={p.id} className="list-row">{p.name}</div>)}
      </article>
      ...
      <article className="glass-card stack">
        ...
        {!reminders.length ? <div className="empty-state">No reminders yet.</div> : reminders.map((r) => <div key={r.id} className="list-row"><span>{r.title}</span><span>{r.date}</span></div>)}
      </article>
