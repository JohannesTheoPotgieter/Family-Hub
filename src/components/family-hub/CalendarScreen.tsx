...
export const CalendarScreen = ({ events, payments, onAddEvent }: Props) => {
  ...
  const dayItems = useMemo(
    () => ({
      ...
      payments: payments.filter((p) => !p.paid && isSameDay(p.dueDate, selectedDate))
    }),
  ...
  return (
    <section className="stack">
      ...
      {dayItems.events.length === 0 && dayItems.payments.length === 0 ? <div className="empty">No events yet</div> : null}
